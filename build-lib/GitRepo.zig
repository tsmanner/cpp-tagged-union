...
pub fn create(b: *std.build.Builder, opt: struct {
    ...
    var result = b.allocator.create(Self) catch @panic("memory");
    ...
    result.* = Self{
        ...
        .branch = opt.branch,
        .sha = opt.sha,
        .path = if (opt.path) |p| (std.fs.path.resolve(b.allocator, &[_][]const u8{
            b.build_root,
            p,
        })) catch @panic("memory") else (std.fs.path.resolve(b.allocator, &[_][]const u8{
            b.build_root,
            "dep",
            name,
        })) catch @panic("memory"),
...
fn run(builder: *std.build.Builder, argv: []const []const u8) !void {
    ...
    switch (result) {
        .Exited => |code| if (code != 0) {
            std.log.err("subprocess failed with exit code {}", .{code});
            std.os.exit(0xff);
        },
        else => {
            std.log.err("subprocess failed with: {}", .{result});
            std.os.exit(0xff);
        },
    }
}

fn checkSha(self: Self) !void {
...
pub fn getPath(self: *const Self, who_wants_to_know: *const std.build.Step) []const u8 {
    if (!hasDependency(who_wants_to_know, &self.step))
        @panic("a step called Self.getPath but has not added it as a dependency");
    return self.path;
}

// Creates a path relative to the repository path and also verifies that the step requesting the path
// is dependent on this step.
pub fn getRelativePath(self: *const Self, who_wants_to_know: *const std.build.Step, path: []const u8) []const u8 {
    return std.fs.path.resolve(self.builder.allocator, &[_][]const u8{ getPath(self, who_wants_to_know), path }) catch @panic("memory");
}
